...
import { default as ApiServerGerminate } from "../components/service/api-server-germinate";
...
export default Service.extend(Evented, {
  ...
  init() {
    ...
    if (isPresent(token)) {
      let /** similar calcs in @see adapters/application.js : host() */
       ...
       * here.
       */
      let primaryServer = this.addServer(
        undefined,
  ...
  addServer: function (typeName = "Pretzel", url, user, token, clientId) {
    ...
    let serverBase = {
      ...
      server = apiServerClass.create(ownerInjection, serverBase),
    ...
    if (existing) console.log("addServer existing=", existing, nameForIndex);
  ...
  serverSelected: computed("controls.serverTabSelected", function () {
     ...
     */
    let serverTabSelectedName = this.get("controls.serverTabSelected"),
  ...
  id2ServerGet(blockId) {
    let id2Server = this.get("id2Server"),
  ...
  id2RemoteRefn(blockId) {
    let id2Server = this.get("id2Server"),
      ...
      remote = isPrimary
        ? blockId
        : { blockId, host: server.host, token: server.token };
  ...
  stores: computed("servers.@each.store", "serversLength", function () {
    let servers = this.get("servers"),
      stores = Object.keys(servers).map((name) => servers[name].store);
    ...
    let servers = this.get("servers"),
      ...
      stores = nameList
        .map((name) => {
          ...
          return { name, server, store: server.store };
        })
        .filter((s) => {
          let d = s.store.peekRecord(fieldName, id);
          return d && Object.assign(s, { object: d });
        });
  ...
  dataset2stores: function (datasetName) {
    let servers = this.get("servers"),
      ...
      stores = nameList
        ...
        .filter((s) => {
          let d = s.store.peekRecord("dataset", datasetName);
          return d && Object.assign(s, { dataset: d });
        });
    if (trace > 2 || (trace && stores.length !== 1))
  ...
  groupId2Server(groupId) {
    let servers = this.get("servers"),
      server = Object.values(servers).find((server) => {
        let groups = server.groups,
          match =
            groups.groupsInIds?.includes(groupId) ||
  ...
  ServerLogin: function (typeName, url, user, password) {
    ...
    const typeIsGerminate = serverTypeIsGerminateAPI(typeName),
      loginP = typeIsGerminate
        ? useGerminate(url, user, password)
        : $.ajax({
            url: url + "/pretzel/api/Clients/login",
            type: "POST",
            crossDomain: true,
      ...
      promise = loginP
        ...
        .catch(function (error) {
          let re = error && error.responseJSON && error.responseJSON.error,
            reTexts =
              re &&
              ["statusCode", "name", "message"].reduce(
                (texts, fN) => (texts[fN] = re[fN]) && texts,
                {},
              );
