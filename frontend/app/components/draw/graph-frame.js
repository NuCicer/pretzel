...
import { DragTransition } from "../../utils/stacks-drag";
...
import { collateData } from "../../utils/draw/collate-paths";
...
*/
  ...
  renderFrame() {
    ...
    let newRender =
      (svgRoot = oa.svgRoot) === undefined ||
      oa.svgRoot.node().getRootNode() !== window.document;
    if (newRender) {
      if (oa.svgRoot)
        console.log(
          "newRender old svgRoot",
          oa.svgRoot.node(),
          oa.svgContainer.node(),
          oa.foreground.node(),
        );
    ...
    let translateTransform =
      "translate(" +
      margins[marginIndex.left] +
      "," +
      margins[marginIndex.top] +
      ")";
    if (newRender) {
      let graphDim = oa.vc.graphDim;
      oa.svgRoot = svgRoot = d3
        .select("#holder")
        .append("svg")
      ...
      let resizeThis =
        // this.resize.bind(oa);
        (transition) => {
          if (trace) dLog("resizeThis", transition);
      ...
      function resizeThisWithTransition() {
        resizeThis(true);
      }
      function resizeThisWithoutTransition() {
        resizeThis(false);
      }

      // This detects window resize, caused by min-/max-imise/full-screen.
      if (true)
        d3.select(window).on("resize." + moduleName, resizeThisWithTransition);
      // also works, can drop if further testing doesn't indicate one is better.
      else
  ...
  renderForeground() {
    ...
    let newRender;
    if (
      oa.foreground === undefined ||
      (newRender = oa.foreground.node().getRootNode() !== window.document)
    ) {
      oa.foreground = oa.svgContainer
        .append("g") // foreground has as elements "paths" that correspond to features
        .attr("class", "foreground");
      let flowValues = d3.values(oa.flows),
        flowsg = oa.foreground
          .selectAll("g")
          .data(flowValues)
          .enter()
          .append("g")
  ...
  showResize(widthChanged, heightChanged, useTransition) {
    ...
    console.log(
      "showResize",
      this.get("viewportWidth"),
      vc.viewPort.w,
      this.get("viewportHeight"),
      vc.viewPort.h,
    );
    ...
    let duration = useTransition || useTransition === undefined ? 750 : 0,
    ...
    if (heightChanged) axisApi.stacksAdjustY(t);

    // for stacked axes, window height change affects the transform.
    if (widthChanged || heightChanged) {
      t.selectAll(".axis-outer").attr(
        "transform",
        Stack.prototype.axisTransformO,
      );
    ...
    if (heightChanged) {
      // let traceCount = 1;
      oa.svgContainer
        .selectAll("g.axis-all > g.brush > clipPath > rect")
        .each(function (d) {
          const a = d,
            ya = a.y,
            yaRange = ya.range();
      ...
      oa.svgContainer
        .selectAll("g.axis-all > g.brush > g[clip-path]")
        .each(function (d) {
          /** d is axis-1d */
          if (d.isDestroying) {
            return;
  ...
  updateSelections() {
    ...
    console.log(
      ...
      svgRoot._groups[0][0],
      ", svgContainer",
      svgContainer._groups[0][0],
      ", foreground",
      foreground._groups[0][0],
    );
  ...
  readLeftPanelShown() {
    later(() => {
      $(".left-panel-shown").on("toggled", (event) =>
        this.readLeftPanelToggle(),
      );
  ...
  resizeEffect: computed(
     ...
     * has changed).
     */
    "stacksWidthChanges.@each",
    "viewportWidth",
    "viewportHeight",
    function () {
      let stacksWidthChanges = this.get("stacksWidthChanges"),
      ...
      if (false)
        // currently the display is probably smoother with the debounce; later after tidying up the resize structure this direct call may be better.
        this.resize.apply(this, [/*transition*/ true]);
      else debounce(this, this.resize, [/*transition*/ true], 500);
      return result;
    },
  ),
  ...
  stacksWidthChanges: computed(
    "blockService.stacksCount",
    "stacksView.splitAxesLength",
    ...
    function () {
      ...
      const leftPanelShown = this.readLeftPanelToggle(),
        current = {
          stacksCount: count,
  ...
  resize: observer(
    ...
    function () {
       ...
       */
      const calledFromObserve = arguments.length === 2,
      ...
      function resizeDrawing() {
        ...
        vc.calc(oa);
        const drawMap = oa.eventBus,
          widthChanged =
            vc.viewPort.w != vc.viewPortPrev.w || this.stacksWidthChanged(),
